...
for year in range(year_start, year_end + 1):
     for c in countries:
         ...
         if charging:
             ...
             dict_charge_depth[call.get_alpha3(c)] = pp.Resample(Charge_depth_profile_utc).loc[:,'Series']
             dict_discharge_depth[call.get_alpha3(c)] = pp.Resample(Discharge_depth_profile_utc).loc[:,'Series']
             dict_charge_limit[call.get_alpha3(c)] = pp.Resample(Charging_limit_profile_utc).loc[:,'Series']
             dict_charge_profile[call.get_alpha3(c)] = pp.Resample(Charging_profile_cap_utc).loc[:,'Series']
             dict_usage[call.get_alpha3(c)] = pp.Resample(pp.usage_prof_norm(Usage_utc, User_list)).loc[:,'Usage']
             
         print('\nExecution Time:', datetime.now() - startTime)

     # Remove local time specification
     df_charge_depth = pd.DataFrame(dict_charge_depth).tz_localize(None)
     dict_discharge_depth = pd.DataFrame(dict_discharge_depth).tz_localize(None)
     dict_charge_limit = pd.DataFrame(dict_charge_limit).tz_localize(None)
     dict_charge_profile = pd.DataFrame(dict_charge_profile).tz_localize(None)
     dict_usage = pd.DataFrame(dict_usage).tz_localize(None)

     # Export the timeseries per each simulated country in one csv file
     pp.export_csv('charge_depth',df_charge_depth,'Europe', simulation_name)
     pp.export_csv('discharge_depth',dict_discharge_depth,'Europe', simulation_name)
     pp.export_csv('charge_limit',dict_charge_limit,'Europe', simulation_name)
     pp.export_csv('charge_profile',dict_charge_profile,'Europe', simulation_name)
     pp.export_csv('usage',dict_usage,'Europe', simulation_name)
